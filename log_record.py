...
class LogRecord(object):
    ...
    def get_section(self):
        url = furl(self.request.split(" ")[1])
        return "%s/%s" % (url.origin, str(url.path).split("/")[1])
        

    def __str__(self):
        return "%s, %s, %s, %s, %s, %s, %s, %s" % (self.remote_host, self.request, self.user_identifier, self.user, self.datetime, self.timezone, self.status_code, self.size)